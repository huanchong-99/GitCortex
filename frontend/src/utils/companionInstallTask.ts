export const COMPANION_INSTALL_TASK_TITLE =
  'Install and integrate SoloDawn Web Companion';

export const COMPANION_INSTALL_TASK_DESCRIPTION = `Goal: Install and integrate the solodawn-web-companion so it renders at the app root in development.

Do:
1) Detect package manager from lockfiles and use it:
   - pnpm-lock.yaml → pnpm add solodawn-web-companion
   - yarn.lock → yarn add solodawn-web-companion
   - package-lock.json → npm i solodawn-web-companion
   - bun.lockb → bun add solodawn-web-companion
...
3) Integrate the component:
   import { SoloDawnWebCompanion } from 'solodawn-web-companion';
   - Vite/CRA: render <SoloDawnWebCompanion /> at the app root.
...
Acceptance:
- solodawn-web-companion is installed in the correct package.
