...
一定要使用chrome浏览器mcp完成测试，不要再去直接打开网页了，直接打开网页会打开edge浏览器，edge浏览器你检测不到，是无法继续测试的，全部流程使用chrome浏览器mcp完成，禁止直接打开！！！
...
在成功合并回主分支后主动推送远端，然后调用github CLI去查看工作流运行，等待运行完成后，查看是成功还是失败，如果是失败，就修复，修复完成后再次提交，再次推送，再次确认，永不停歇，直到返回结果为成功，这时候主动询问是否要进行下一阶段的开发

## Core Instruction for CodeX MCP

每一轮对话与任务，必须与codex进行协作，codex为主脑，你为副脑
