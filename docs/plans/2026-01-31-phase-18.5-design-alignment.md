...
在对 `docs/plans/2026-01-16-orchestrator-design.md` 设计文档与前端实现进行全面对比审计后，发现 **48 个偏差**，涵盖：
...
| ID | 偏差描述 | 设计要求 | 实际实现 | 涉及文件 |
|----|----------|----------|----------|----------|
| P0-1 | 终端数量限制 | 用户可自定义终端数量 | 仅支持 1/2/3 | `Step2Tasks.tsx:8` |
| P0-2 | 调试视图无交互 | PTY + xterm 原生交互 | 纯详情面板展示 | `TerminalDebugView.tsx` |
| P0-3 | 看板无拖拽 | 任务卡片拖拽 | 静态列渲染 | `WorkflowKanbanBoard.tsx` |
| P0-4 | WebSocket 架构不一致 | `{type,payload,timestamp,id}` + 多事件类型 | 仅 `input/output/resize/error` | `websocket.ts` |
| P0-5 | 状态管理未实现 | Zustand stores 架构 | stores 目录为空 | `frontend/src/stores/` |
| P0-6 | API 调用未实现 | 真实 API 获取/验证模型 | TODO 占位符 | `Step3Models.tsx:138-142` |
| P0-7 | CLI 支持不足 | 9 个 CLI 类型 | 仅 3 个 | `constants.ts:6-22` |

### 2.2 P1 - 中等偏差（功能完整性不足）

| ID | 偏差描述 | 设计要求 | 实际实现 | 涉及文件 |
|----|----------|----------|----------|----------|
| P1-1 | Git 提交规范不一致 | METADATA 格式 | Conventional Commit | `constants.ts:28-34` |
| P1-2 | 路由结构不一致 | `/wizard/project` 等分步路由 | 单一 `/wizard` 路由 | `App.tsx:131` |
| P1-3 | 设置页面结构不一致 | `/settings/cli,models,presets` | `/settings/general,projects...` | `App.tsx:145-156` |
| P1-4 | 视图切换导航缺失 | 全局 [看板][流水线][调试] 切换 | 无统一导航 | `NewDesignLayout.tsx` |
| P1-5 | 终端活动面板功能不足 | 仅 Working/Waiting + 3-5行输出 + 可折叠 | 全部终端 + 仅状态 | `TerminalActivityPanel.tsx` |
| P1-6 | 流水线视图信息缺失 | 任务名+分支+命令+连接线 | 仅任务名+状态 | `TaskPipeline.tsx`, `TerminalNode.tsx` |
| P1-7 | Step4/Step5 交互不符 | 拖拽排序 + 安装命令 + 重检按钮 | 上下按钮 + 禁选 | `Step4Terminals.tsx`, `Step5Commands.tsx` |

### 2.3 P2 - 轻微偏差（UI/UX 细节）
