...
**计划文件:** `2026-01-21-single-project-migration.md`

| Task | 目标描述 | 状态 | 完成时间 |
|------|----------|------|----------|
| 11.1 | 冻结迁移清单（Keep/Drop/补齐） | ⬜ |  |
| 11.2 | cc-switch-main 必要模块补齐迁入 | ⬜ |  |
| 11.3 | 一次性迁移核心目录/配置到根目录 | ⬜ |  |
| 11.4 | 路径与工作区配置重写（Cargo/pnpm/scripts） | ⬜ |  |
| 11.5 | 删除不需要模块/目录（remote-frontend、npx-cli、dev_assets_seed、上游 docs 等） | ⬜ |  |
| 11.6 | 删除源目录（vibe-kanban-main、cc-switch-main） | ⬜ |  |
