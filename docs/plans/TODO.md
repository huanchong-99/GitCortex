...
**计划文件:** `2026-02-06-phase-25-auto-confirm-fix.md`
...
| Task | 目标描述 | 状态 | 完成时间 |
|------|----------|------|----------|
| 25.1 | 前端 workflow 创建请求 terminal 增加 `autoConfirm`，默认 `true` | ⬜ |  |
| 25.2 | 后端 `auto_confirm` 缺省值改为 `true`（保留显式 `false`） | ⬜ |  |

### P1 - PromptWatcher 后台解耦

| Task | 目标描述 | 状态 | 完成时间 |
|------|----------|------|----------|
| 25.3 | 建立 PTY 输出后台扇出通道，解耦 WS 与 PromptWatcher | ⬜ |  |
| 25.4 | PromptWatcher 改为独立后台任务，覆盖 workflow/手动启动路径 | ⬜ |  |
...
| 25.6 | 增强链路日志与端到端回归（有 WS / 无 WS / 断连重连） | ⬜ |  |
