...
> **说明:** 此 Phase 为 2026-01-19 代码审计后新增，优先级最高，必须完成后才能继续其他开发。

### P0 - 严重问题修复 (生产环境阻塞)

| Task | 目标描述 | 状态 | 完成时间 |
|------|----------|------|----------|
| 8.5.1 | 实现 execute_instruction 核心逻辑 - 移除 TODO 占位符 | ⬜ | - |
| 8.5.2 | API Key 加密存储 - 使用 AES-256-GCM 加密敏感字段 | ⬜ | - |
| 8.5.3 | 实现 handle_git_event 实际逻辑 - Git 事件到终端完成事件转换 | ⬜ | - |

### P1 - 代码清理

| Task | 目标描述 | 状态 | 完成时间 |
|------|----------|------|----------|
| 8.5.4 | 移除未使用的导入 - 清理 7 个编译警告 | ⬜ | - |
| 8.5.5 | 移除/使用未使用的 db 字段 - OrchestratorAgent.dead_code | ⬜ | - |
| 8.5.6 | 统一命名规范 - Rust snake_case, TypeScript camelCase, serde rename_all | ⬜ | - |
| 8.5.7 | 添加错误重试机制 - LLM 请求网络错误重试 (最多3次) | ⬜ | - |

### P2 - 代码重构

| Task | 目标描述 | 状态 | 完成时间 |
|------|----------|------|----------|
| 8.5.8 | 重构魔法数字 - MAX_HISTORY 等改为可配置项 | ⬜ | - |
| 8.5.9 | 重构硬编码字符串 - 提取常量 (WORKFLOW_TOPIC_PREFIX 等) | ⬜ | - |
| 8.5.10 | 完善状态机转换 - 显式状态转换，验证合法性 | ⬜ | - |
| 8.5.11 | LLM 提示词模板化 - 使用 Handlebars 模板引擎 | ⬜ | - |
| 8.5.12 | 数据库批量操作优化 - 使用事务批量插入 | ⬜ | - |
| 8.5.13 | WebSocket 终端连接超时控制 | ⬜ | - |
