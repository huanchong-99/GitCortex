...
**计划文件:** `2026-01-23-phase-12-api-contract.md`

| Task | 目标描述 | 状态 | 完成时间 |
|------|----------|------|----------|
| 12.1 | 冻结 Workflow API 契约（请求/响应/状态枚举） | ⬜ |  |
| 12.2 | 后端 DTO/serde 对齐与响应重构 | ⬜ |  |
| 12.3 | 生成类型对齐（generate_types / shared/types.ts） | ⬜ |  |
| 12.4 | 前端 hooks/types 对齐（useWorkflows 等） | ⬜ |  |
| 12.5 | 状态枚举统一与映射修复（workflow/task/terminal） | ⬜ |  |
| 12.6 | 契约测试与回归验证（API + 前端） | ⬜ |  |
...
| Task | 目标描述 | 状态 | 完成时间 |
|------|----------|------|----------|
| 13.1 | 扩展 CreateWorkflowRequest 支持任务与终端配置 | ⬜ |  |
| 13.2 | 使用事务创建 workflow + tasks + terminals | ⬜ |  |
| 13.3 | 任务分支命名/冲突策略与默认规则 | ⬜ |  |
| 13.4 | CLI/模型配置校验与错误返回规范化 | ⬜ |  |
| 13.5 | WorkflowCommand 关联与自定义参数支持 | ⬜ |  |
| 13.6 | Workflow Detail/List 返回完整任务/终端/命令 | ⬜ |  |
| 13.7 | API 集成测试与回滚验证 | ⬜ |  |
...
| Task | 目标描述 | 状态 | 完成时间 |
|------|----------|------|----------|
| 14.1 | Orchestrator 运行时服务装配（Deployment 容器） | ⬜ |  |
| 14.2 | start_workflow 触发编排流程与状态迁移 | ⬜ |  |
| 14.3 | 终端启动序列（cc-switch 串行启动/并行运行） | ⬜ |  |
| 14.4 | 任务/终端状态更新与事件广播 | ⬜ |  |
| 14.5 | GitWatcher 事件驱动接入 Orchestrator | ⬜ |  |
| 14.6 | Merge Terminal 合并流程与冲突处理 | ⬜ |  |
| 14.7 | Error Terminal 异常处理流程 | ⬜ |  |
| 14.8 | 运行态持久化与恢复（重启续跑） | ⬜ |  |
...
| Task | 目标描述 | 状态 | 完成时间 |
|------|----------|------|----------|
| 15.1 | Workflow Terminal 与 Session/ExecutionProcess 绑定 | ⬜ |  |
| 15.2 | PTY 进程生命周期管理与 WebSocket 转发 | ⬜ |  |
| 15.3 | 终端输出持久化与历史回放 | ⬜ |  |
| 15.4 | 终端超时/取消/清理策略 | ⬜ |  |
| 15.5 | CLI 检测与安装指引联动 UI | ⬜ |  |
| 15.6 | 终端相关单测/集成测试完善 | ⬜ |  |
...
**计划文件:** `2026-01-23-phase-16-frontend-workflow-ux.md`

| Task | 目标描述 | 状态 | 完成时间 |
|------|----------|------|----------|
| 16.1 | Workflows 列表/详情改为真实任务数据 | ⬜ |  |
| 16.2 | PipelineView 显示真实任务/终端状态 | ⬜ |  |
| 16.3 | WorkflowDebug 实时终端调试接入 | ⬜ |  |
| 16.4 | WorkflowWizard 提交 payload/校验对齐 | ⬜ |  |
| 16.5 | 启动/暂停/停止控制与权限提示 | ⬜ |  |
| 16.6 | i18n/错误态/空态完善 | ⬜ |  |
...
| 17.1 | 斜杠命令预设 CRUD（后端） | ⬜ |  |
| 17.2 | 前端斜杠命令管理与选择 | ⬜ |  |
| 17.3 | WorkflowCommand 关联排序与参数编辑 | ⬜ |  |
| 17.4 | Orchestrator 提示词渲染与命令执行 | ⬜ |  |
| 17.5 | 命令系统测试与回归 | ⬜ |  |
...
| 18.4 | 安全与配置审计（密钥/权限/日志） | ⬜ |  |
| 18.5 | 使用文档与运维手册完善 | ⬜ |  |
| 18.6 | 发布与回滚清单（版本/迁移/备份） | ⬜ |  |
