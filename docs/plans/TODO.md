...
| 已完成 | 122 |
| 进行中 | 0 |
| 未开始 | 36 (Phase 18: 6个, Phase 18.1: 15个, Phase 19: 6个) |
| 可选优化 | 0 |
| **完成率** | **77.2%** |
...
>
> **技术债务来源（Codex MCP 分析）:**
> - 9 个被忽略的库测试（slugify、template_renderer、terminal_coordinator、orchestrator）
> - 12 个集成测试文件有编译错误（SqlitePoolOptions 导入、类型注解、借用检查）
...
| Task | 目标描述 | 状态 | 完成时间 |
|------|----------|------|----------|
| 18.1.6 | 修复 slugify 函数 - 特殊字符替换为连字符 | ⬜ |  |
| 18.1.7 | 修复 slugify 函数 - 使用 `is_ascii_alphanumeric()` 排除 CJK | ⬜ |  |
| 18.1.8 | 修复 test_render_missing_variable - 更新断言字符串 | ⬜ |  |
| 18.1.9 | 统一 commit metadata 格式 - 同步解析逻辑或测试用例 | ⬜ |  |

### P2 - 测试基础设施重构

| Task | 目标描述 | 状态 | 完成时间 |
|------|----------|------|----------|
| 18.1.10 | 重构 terminal_coordinator_test DB 初始化 - 使用真实迁移 | ⬜ |  |
| 18.1.11 | 补齐 terminal_coordinator_test seed 数据 | ⬜ |  |
| 18.1.12 | 修复测试顺序依赖 | ⬜ |  |
...
| Task | 目标描述 | 状态 | 完成时间 |
|------|----------|------|----------|
| 18.1.13 | 移除 `#[ignore]` 属性 - 逐步解禁已修复测试 | ⬜ |  |
| 18.1.14 | 恢复 CI 完整测试 - 将 `--lib` 改回完整测试 | ⬜ |  |
| 18.1.15 | 全量回归验证 | ⬜ |  |
