...
| 已完成 | 241 |
| 进行中 | 0 |
| 未开始 | 48 (Phase 18.5: 3个可选, Phase 21: 2个可选, Phase 24: 43个, Phase 25: 6个) |
| 可选优化 | 5 |
| **完成率** | **83.4%** |
...
| 24.3 | 处理行尾补齐（无 `\n` 时自动追加，Windows 下使用 `\r\n`） | ⬜ |  |
| 24.4 | 处理写入失败与终端不存在的情况 | ⬜ |  |
| 24.5 | 维护活跃 session 的 map，终端退出时清理 | ⬜ |  |
...
| 24.11 | 增加全局或 per-workflow 的自动确认开关 | ⬜ |  |
| 24.12 | 确保手动启动路径也带上参数 | ⬜ |  |
...
| Task | 目标描述 | 状态 | 完成时间 |
|------|----------|------|----------|
| 24.13 | 创建 `terminal/approval_detector.rs` 模块 | ⬜ |  |
| 24.14 | 实现 CLI 权限/交互提示检测 | ⬜ |  |
| 24.15 | 检测触发后将 terminal 状态切到 `waiting_for_approval` | ⬜ |  |
| 24.16 | 广播 `TerminalStatusUpdate` 事件 | ⬜ |  |
| 24.17 | 若启用自动确认，自动发送 `y\n` 并恢复到 `working` | ⬜ |  |
| 24.18 | 若未启用自动确认，提示用户进入终端确认 | ⬜ |  |
| 24.19 | 维护每个 terminal 的 approval 状态机 | ⬜ |  |

### P2 - 超时告警机制

| Task | 目标描述 | 状态 | 完成时间 |
|------|----------|------|----------|
| 24.20 | 在 PTY 输出监控里记录 `last_output_at` | ⬜ |  |
| 24.21 | 超过阈值（默认 120s）无输出时标记为 `stalled` 状态 | ⬜ |  |
| 24.22 | 写入系统日志并广播 `TerminalStatusUpdate` | ⬜ |  |
| 24.23 | 输出恢复时自动回到 `working` | ⬜ |  |
| 24.24 | 新增数据库迁移 `add_terminal_last_output_at.sql` | ⬜ |  |
| 24.25 | 前端显示 `stalled` 状态的 UI | ⬜ |  |

### P3 - 测试与回归

| Task | 目标描述 | 状态 | 完成时间 |
|------|----------|------|----------|
| 24.26 | 新增 MessageBus → PTY 桥接单测 | ⬜ |  |
| 24.27 | 新增 CLI 自动确认参数单测 | ⬜ |  |
| 24.28 | 新增等待确认状态检测单测 | ⬜ |  |
| 24.29 | 新增超时告警机制单测 | ⬜ |  |
| 24.30 | 端到端测试：工作流创建 -> 终端启动 -> 自动确认 -> 命令执行 | ⬜ |  |
| 24.31 | CI 回归验证 | ⬜ |  |

---

## Phase 25: Docker 容器化部署（开源发布优化）
...
| Task | 目标描述 | 状态 | 完成时间 |
|------|----------|------|----------|
| 25.1 | 创建生产部署 Dockerfile（多阶段构建） | ⬜ |  |
| 25.2 | 创建生产部署 Docker Compose | ⬜ |  |
| 25.3 | 创建开发调试 Docker Compose（可选） | ⬜ |  |
| 25.4 | 优化 Docker 镜像构建（缓存/体积） | ⬜ |  |
| 25.5 | 创建 Docker 部署文档 | ⬜ |  |
| 25.6 | CI/CD 集成（GitHub Actions） | ⬜ |  |
